...
- **D-13:** **Wrapper component for the keyed route content** = inline `<motion.div key={location.pathname} variants={pageFade} initial="hidden" animate="visible" exit="exit">` sitting inside `<AnimatePresence>` and containing `<Outlet/>`. Uses dedicated `exit` state from `pageFade` for SC#3's 350ms exit — `hidden` would skip the timing. No new file needed; the entire Phase 5 route-transition surface is ~10 lines of edits inside `Layout.tsx`.
