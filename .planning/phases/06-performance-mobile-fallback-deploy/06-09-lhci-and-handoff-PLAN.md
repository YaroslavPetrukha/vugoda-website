...
must_haves:
  truths:
    ...
    - "docs/CLIENT-HANDOFF.md exists listing the 4 file edits required if the user's GitHub account ≠ 'yaroslavpetrukha' (D-26 + D-38 + RESEARCH §Risk 1) — index.html (4 occurrences), .lighthouserc.cjs (5 occurrences), src/content/mobile-fallback.ts (1 Lakeview link — does NOT change with account), src/data/projects.ts (Lakeview externalUrl — does NOT change with account)"
  artifacts:
